    ...
    public class EnemyCharacter : MonoBehaviour
    {
        ...
        protected virtual void CheckGround()
        {
            ...
            if (rightward)
            {
                ...
                //Debug.DrawRay(forwardRay.origin, transform.right, Color.green,.1f);
            }
            else
            {
                forwardHit = Physics2D.Raycast(forwardRay.origin, -transform.right, .1f);
            ...
            for(int i = 0; i<3; i++)
            {
                hits[i] = Physics2D.Raycast(groundRays[i].origin, -transform.up, .5f);
                //Debug.DrawRay(groundRays[i].origin, -transform.up, Color.green, .5f);
            }
            int numberOfHits = 0;
